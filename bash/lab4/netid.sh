...
default_router_address=$(ip r s default| awk '{print $3}3')
...
#identify the default interface name
interface_name=($(ip route |grep -w "default" | awk '{print $5}'))

#if cannot find default ip route, we use ens33 as default
...
[ "$verbose" = "yes" ] && echo "report on interface: $interface"
[ "$verbose" = "yes" ] && echo "getting ipv4 address and interface name: $interface"
